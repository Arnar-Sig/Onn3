...
(define (foldr f x u)
  (define (hjalp rev u)
    (if (null? rev)
        u
        (hjalp (cdr rev) (f u (car rev))))
  )
  (hjalp (reverse x) u)
)


;Dæmi 2 test ÖRUGGLEGA RANGT
(define (foldr2 f x u)
  (define (hjalp u rev)
    (if (null? (cdr rev))
        (car rev)
        (f (hjalp f (cdr rev))(car rev) ))
  )
    (hjalp u (reverse x))
 )


;Dæmi 2 test 2 - reyna herma eftir foldl, dæmi 2 í einst
(define (foldr3 f x u)
  (define (hjalp u rev)
    (if (null? (cdr rev))
        u
        (hjalp (f u (cdr rev)) (cdr rev))
    )
  )(hjalp u (reverse x))
)
