...
(define (mulstreams x y)
  ;; Notkun: hjalp
  ;; Fyrir:  y er óendanlegur straumur talna,
  ;;         y=[x1 x2 x3 ...].
  ;; Gildi:  Óendanlegur straumur af tölunni x1,
  ;;         y=[x1 x1 x1 ...].
  (define hjalp
    (cons-stream (stream-car y) hjalp)
  )
  (cons-stream (stream-binop * x hjalp) (mulstreams x (stream-cdr y)))
)
...
;;          .
;;         ]
(define (squarestream s)
  ;; Notkun: (hjalp s x)
  ;; Fyrir:  s er óendanlegur straumur talna,
  ;;         s=[a1 a2 a3 ...].
  ;;         x er óendanlegur straumur talna,
  ;;         x=[b1 b2 b3 ...].
  ;; Gildi:  Óendanlegur straumur óendanlegra strauma
  ;;         talna sem er
  ;;         [[a1*b1 a2*b1 a3*b1 ...]
  ...
  (define (hjalp s x)
    ...
    ;;         y=[x1 x1 x1 ...].
      (define hjalp2
        (cons-stream (stream-car x) hjalp2))
    (cons-stream (stream-binop * s hjalp2) (hjalp s (stream-cdr x)))
  )
  (hjalp s s)
)
...
;;            .
;;           ]
;; Gildi:  Óendanlegi straumurinn sem er
;;         byltingin (transpose) af s, þ.e.
;;         [[x11 x21 x31 ...]
;;          [x12 x22 x32 ...]
;;          [x13 x23 x33 ...]
;;          .
...
(define (byltastraumi s)
  (cons-stream (stream-map stream-car s) (byltastraumi (stream-map stream-cdr s)))
)
