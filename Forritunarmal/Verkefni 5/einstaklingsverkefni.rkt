...
;Dæmi 1
;; Notkun: (mypow x n)
;; Fyrir:  heiltölur x, n þar sem n≥0
;; Gildi:  Gildið x í veldinu n eða x^n.    
(define (mypow x n)
  ;; Notkun: (hjalp p x n r q)
  ;; Fyrir:  p er fleytitala
  ;;         x er heiltala
  ;;         n er heiltala, þar sem n≥0
  ;;         r er heiltala, þar sem 0 ≤ r ≤ n
  ;;         q er heiltala, þar sem q≥0      
  ;; Gildi: gildi p = x í veldinu n eða x^n.
  (define (hjalp p x n r q)
    (if(< r 1)
       p
       (if(= (remainder r 2) 0)
          (hjalp p x n (/ r 2) (* q q))
...
(define (byltalista z)
  (if(null? z)
     ...
     (if(null? (car z))
        '()
        (cons (map car z) (byltalista (map cdr z))))))