...
(define (mulstreams x y)
  (cons-stream (stream-map (lambda (z) (* z (stream-car y)))x)
...
(define (squarestream s)
  (define (hjalp s x)
    (cons-stream (stream-map (lambda (z) (* (stream-car x) z))s) (hjalp s (stream-cdr x)))
  )
  (hjalp s s)
)
...
(define (byltastraumi s)
  (cons-stream (stream-map (stream-car) s)
...
(define (byltalista s)
  (if (or (null? s) (null? (cdr s)))
      '()
      (cons (map car s) (byltalista (map cdr s)))))
